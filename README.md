...
| Ensuring focus on individuals employed at the time of survey         | Identified indicator of job status and filtered the dataset to focus on those in employment and not doing a degree at the time of survey. |
| Handling missing values and mismatches             | Ensured no missing key columns and values, especially artificial missing values generated by inconsistent variable names in the raw data, using for loops and assertions to help with this. |
...
*Here's a sneak peak (click thumbnail below to watch a demo)*:
